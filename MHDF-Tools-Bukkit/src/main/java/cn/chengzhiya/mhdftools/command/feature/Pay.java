...
final class Pay extends AbstractCommand {
    public Pay() {
        super(
                ...
                true,
                Main.instance.getConfigManager().getConfigManager().getData().getStringList("economySettings.payCommands").toArray(new String[0])
        );
    }
    ...
    public void execute(@NotNull Player sender, @NotNull String label, @NotNull String[] args) {
        if (args.length != 2) {
            ActionUtil.sendMessage(sender, Main.instance.getConfigManager().getLangManager().i18n("usageError")
                    .replace("{usage}", Main.instance.getConfigManager().getLangManager().i18n("commands.pay.usage"))
        ...
        if (args[0].equals(sender.getName())) {
            ActionUtil.sendMessage(sender, Main.instance.getConfigManager().getLangManager().i18n("commands.pay.paySelf"));
            return;
        }
        ...
        try {
            ...
            ActionUtil.sendMessage(sender, Main.instance.getConfigManager().getLangManager().i18n("commands.pay.moneyFormatError"));
            return;
        }
        ...
        if (mhdfPayPlayer.getMoney().compareTo(amount) < 0) {
            ActionUtil.sendMessage(sender, Main.instance.getConfigManager().getLangManager().i18n("commands.pay.noMoney"));
            return;
        }
        ...
        if (Main.instance.getConfigManager().getConfigManager().getData().getBoolean("economySettings.personalIncomeTax.enable")) {
            tax = amount.multiply(BigDecimalUtil.toBigDecimal(
                    Main.instance.getConfigManager().getConfigManager().getData().getDouble("economySettings.personalIncomeTax.rate"))
            );
        ...
        ActionUtil.sendMessage(sender, Main.instance.getConfigManager().getLangManager().i18n("commands.pay.message")
                .replace("{player}", MHDFToolsAPIHelper.getInstance().getPlayerManager().getPlayer(player).getDisplayName())
                .replace("{amount}", String.valueOf(amount))
        );

        ActionUtil.sendMessage(player.getPlayer(), Main.instance.getConfigManager().getLangManager().i18n("commands.pay.receivedMessage")
