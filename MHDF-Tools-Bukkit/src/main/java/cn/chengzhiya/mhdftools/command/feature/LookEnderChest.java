...
public final class LookEnderChest extends AbstractCommand {
    public LookEnderChest() {
        super(
                ...
                true,
                ConfigUtil.getConfig().getStringList("chatSettings.showEnderChest.commands").toArray(new String[0])
        );
    }
    ...
    public void execute(@NotNull Player sender, @NotNull String label, @NotNull String[] args) {
        if (args.length != 1) {
            ActionUtil.sendMessage(sender, LangUtil.i18n("usageError")
                    .replace("{usage}", LangUtil.i18n("commands.lookenderchest.usage"))
        ...
        if (data == null) {
            ActionUtil.sendMessage(sender, LangUtil.i18n("commands.lookenderchest.noData"));
            return;
        }
