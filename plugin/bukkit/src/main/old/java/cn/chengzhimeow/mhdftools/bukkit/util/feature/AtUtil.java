...
public final class AtUtil {
    ...
    public static Set<String> getAtList(Player player, String message) {
        ...
        for (VanishStatus vanishStatus : MHDFToolsAPI.getInstance().getVanishStatusManager().getList()) {
            ...
            MHDFToolsPlayer mhdfVanishPlayer = MHDFToolsAPI.getInstance().getPlayerManager().getPlayer(vanishStatus.getPlayer());
    ...
    public static void at(Player player, String by) {
        ...
        MHDFToolsPlayer mhdfByPlayer = MHDFToolsAPI.getInstance().getPlayerManager().getPlayer(byPlayer);
        if (mhdfPlayer.isIgnore(mhdfByPlayer)) return;

        // 发送AT提示
        String title = LangSetting.getInstance().getData().getString("chat.at.title")
                .replace("{by}", by);
        if (!title.isEmpty()) {
        ...
        // 播放音效
        String sound = SoundSetting.getInstance().getData().getString("chat.at");
